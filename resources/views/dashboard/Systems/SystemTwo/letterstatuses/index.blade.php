@section('title')
    {{__('Systems/SystemTwo/letterstatuses.letterstatuses')}}
@endsection
...
<div class="breadcrumbbar">
    <div class="row align-items-center">
        <div class="col-md-8 col-lg-8">
            <h4 class="page-title">{{__('Systems/SystemTwo/letterstatuses.letterstatuses_list')}}</h4>
            <div class="breadcrumb-list">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{url('/home')}}">{{ __('side.dashboard') }}</a></li>
                    <li class="breadcrumb-item">{{__('Systems/SystemTwo/letterstatuses.letterstatuses')}}</li>
                    <li class="breadcrumb-item active" aria-current="page">{{__('Systems/SystemTwo/letterstatuses.letterstatuses_list')}}</li>
                </ol>
            </div>
        </div>
        <div class="col-md-4 col-lg-4">
            <div class="widgetbar">
                <a href="{{ route('letterstatuses.create') }}" class="btn btn-primary-rgba"><i class="feather icon-plus mr-2"></i>{{__('Systems/SystemTwo/letterstatuses.add_letterstatus')}}</a>
            </div>
        </div>
    </div>
</div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-borderless" id="default-datatable">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>{{__('Systems/SystemTwo/letterstatuses.name')}}</th>
                                    <th>{{__('Systems/SystemTwo/letterstatuses.person')}}</th>
                                    <th>{{__('Systems/SystemTwo/letterstatuses.state')}}</th>
                                    <th>{{__('Systems/SystemTwo/letterstatuses.edit')}}</th>
                                    <th>{{__('Systems/SystemTwo/letterstatuses.delete')}}</th>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if(app()->getLocale() == 'en')
                                                    {{$letterstatus->name}}
                                                @else
                                                    {{$letterstatus->ar_name}}
                                                @endif
                                            </td>
                                            <td>{{ $letterstatus->person }}</td>
                                            <td>
                                                <div class="custom-control custom-switch" >
                                                    <input type="checkbox" onclick="status_change('{{csrf_token()}}','{{$letterstatus->id}}','{{url('dashboard/letterstatus-state')}} ')" {{ $letterstatus->state == 1 ? 'checked':''}} class="custom-control-input" id="customSwitch{{$key}}">
                                                    <label class="custom-control-label" for="customSwitch{{$key}}"></label>
                                                </div>
                                            </td>
                                            <td><a href="{{route('letterstatuses.edit', $letterstatus->id)}}" class="btn btn-success-rgba"><i class="feather icon-eye"></i></a></td>
                                            <td><a onclick="deleteConfirm({{$letterstatus->id}})" class="btn btn-danger-rgba"><i class="feather icon-trash"></i></a></td>
                                      </tr>
                                    @endforeach
                                    @endif
                            </tbody>
...
<script>
    ...
    function deleteletterstatus(id) {

        $("#deleteBtn").attr("href", "{{url('dashboard/del-letterstatus')}}/" + id);
...
<script>
    function deleteConfirm(id) {
        ...
        swal({
            ...
            $.ajax({
                method: "post",
                url: "{{url('dashboard/del-letterstatus')}}",
                headers: {
                ...
                success: function (data) {
                    ...
                    if (data === 0) {
                        ...
                        swal(
                            ...
                            window.location = "{{route('letterstatuses.index')}}"
                        });
                    }
                },
                error: function () {
            ...
            if (dismiss === 'cancel') {
                swal(
                    'Cancelled',
                    'Your letterstatus data is safe :)',
                    'error'
                )
            }
        })
