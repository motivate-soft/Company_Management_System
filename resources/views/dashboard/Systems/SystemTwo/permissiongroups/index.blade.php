@section('title')
    {{__('Systems/SystemTwo/permissiongroups.permissiongroups')}}
@endsection
...
<div class="breadcrumbbar">
    <div class="row align-items-center">
        <div class="col-md-8 col-lg-8">
            <h4 class="page-title">{{__('Systems/SystemTwo/permissiongroups.permissiongroups_list')}}</h4>
            <div class="breadcrumb-list">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{url('/home')}}">{{ __('side.dashboard') }}</a></li>
                    <li class="breadcrumb-item">{{__('Systems/SystemTwo/permissiongroups.permissiongroups')}}</li>
                    <li class="breadcrumb-item active" aria-current="page">{{__('Systems/SystemTwo/permissiongroups.permissiongroups_list')}}</li>
                </ol>
            </div>
        </div>
        <div class="col-md-4 col-lg-4">
            <div class="widgetbar">
                <a href="{{ route('permissiongroups.create') }}" class="btn btn-primary-rgba"><i class="feather icon-plus mr-2"></i>{{__('Systems/SystemTwo/permissiongroups.add_permissiongroup')}}</a>
            </div>
        </div>
    </div>
</div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-borderless" id="default-datatable">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>{{__('Systems/SystemTwo/permissiongroups.name')}}</th>
                                    <th>{{__('Systems/SystemTwo/permissiongroups.state')}}</th>
                                    <th>{{__('Systems/SystemTwo/permissiongroups.detail')}}</th>
                                    <th>{{__('Systems/SystemTwo/permissiongroups.edit')}}</th>
                                    <th>{{__('Systems/SystemTwo/permissiongroups.delete')}}</th>
                                    ...
                                    @foreach($permissiongroups as $key => $permissiongroup)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @else
                                                    {{$permissiongroup->ar_name}}
                                                @endif
                                            </td>
                                            <td>
                                                <div class="custom-control custom-switch" >
                                                    <input type="checkbox" onclick="status_change('{{csrf_token()}}','{{$permissiongroup->id}}','{{url('dashboard/permissiongroup-state')}} ')" {{ $permissiongroup->state == 1 ? 'checked':''}} class="custom-control-input" id="customSwitch{{$key}}">
                                                    <label class="custom-control-label" for="customSwitch{{$key}}"></label>
                                                </div>
                                            </td>
                                            <td><a href="{{route('permissiongroups.detail', $permissiongroup->id)}}" class="btn btn-success-rgba"><i class="feather icon-eye"></i></a></td>
                                            <td><a href="{{route('permissiongroups.edit', $permissiongroup->id)}}" class="btn btn-success-rgba"><i class="feather icon-edit-2"></i></a></td>
                                            <td><a onclick="deleteConfirm({{$permissiongroup->id}})" class="btn btn-danger-rgba"><i class="feather icon-trash"></i></a></td>
                                      </tr>
                                    @endforeach
                                    @endif
                            </tbody>
...
<script>
    ...
    function deletepermissiongroup(id) {
        ...
        $("#deleteBtn").attr("href", "{{url('dashboard/del-permissiongroup')}}/" + id);
...
<script>
    function deleteConfirm(id) {
        ...
        swal({
            ...
            $.ajax({
                method: "post",
                url: "{{url('dashboard/del-permissiongroup')}}",
                headers: {
                ...
                success: function (data) {
                    ...
                    if (data === 0) {
                        ...
                        swal(
                            ...
                            window.location = "{{route('permissiongroups.index')}}"
                        });
                    }
                },
                error: function () {
            ...
            if (dismiss === 'cancel') {
                swal(
                    'Cancelled',
                    'Your permissiongroup data is safe :)',
                    'error'
                )
            }
        })
