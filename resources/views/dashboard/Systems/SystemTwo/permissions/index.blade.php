@section('title')
    {{__('Systems/SystemTwo/permissions.permissions')}}
@endsection
...
<div class="breadcrumbbar">
    <div class="row align-items-center">
        <div class="col-md-8 col-lg-8">
            <h4 class="page-title">{{__('Systems/SystemTwo/permissions.permissions_list')}}</h4>
            <div class="breadcrumb-list">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{url('/home')}}">{{ __('side.dashboard') }}</a></li>
                    <li class="breadcrumb-item">{{__('Systems/SystemTwo/permissions.permissions')}}</li>
                    <li class="breadcrumb-item active" aria-current="page">{{__('Systems/SystemTwo/permissions.permissions_list')}}</li>
                </ol>
            </div>
        </div>
        <div class="col-md-4 col-lg-4">
            <div class="widgetbar">
                <a href="{{ route('permissions.create') }}" class="btn btn-primary-rgba"><i class="feather icon-plus mr-2"></i>{{__('Systems/SystemTwo/permissions.add_permission')}}</a>
            </div>
        </div>
    </div>
</div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-borderless" id="default-datatable">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>{{__('Systems/SystemTwo/permissions.name')}}</th>
{{--                                    <th>{{__('Systems/SystemTwo/permissions.state')}}</th>--}}
                                    <th>{{__('Systems/SystemTwo/permissions.edit')}}</th>
                                    <th>{{__('Systems/SystemTwo/permissions.delete')}}</th>
                                        ...
                                        <tr>
                                            ...
                                            {{--<td>--}}
                                                {{--<div class="custom-control custom-switch" >--}}
                                                    {{--<input type="checkbox" onclick="status_change('{{csrf_token()}}','{{$permission->id}}','{{url('dashboard/permission-state')}} ')" {{ $permission->state == 1 ? 'checked':''}} class="custom-control-input" id="customSwitch{{$key}}">--}}
                                                    {{--<label class="custom-control-label" for="customSwitch{{$key}}"></label>--}}
                                                {{--</div>--}}
                                            {{--</td>--}}
                                            <td><a href="{{route('permissions.edit', $permission->id)}}" class="btn btn-success-rgba"><i class="feather icon-eye"></i></a></td>
                                            <td><a onclick="deleteConfirm({{$permission->id}})" class="btn btn-danger-rgba"><i class="feather icon-trash"></i></a></td>
                                      </tr>
                                    @endforeach
                                    @endif
                            </tbody>
...
<script>
    ...
    function deletepermission(id) {
        ...
        $("#deleteBtn").attr("href", "{{url('dashboard/del-permission')}}/" + id);
...
<script>
    function deleteConfirm(id) {
        ...
        swal({
            ...
            $.ajax({
                method: "post",
                url: "{{url('dashboard/del-permission')}}",
                headers: {
