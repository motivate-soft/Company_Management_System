                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-3 mb-4">
                            <div class="form-group mb-0">
                                ...
                                <input type="text" placeholder="name" class="form-control" @if(isset($companydomain->name)) value="{{ $companydomain->name }}"@endif name="name" placeholder="No name" required="">
                            </div>
                        </div>
                                ...
                                <select id="person" name="person" class="form-control">
                                    @if(isset($employees) && count($employees))
                                        @foreach($employees as $key => $employee)
                                            <option  @if(isset($companydomain->person) && $companydomain->person == $employee->firstname) selected @endif value="{{$employee->firstname}}">{{$employee->firstname}}</option>
                                        @endforeach
                                    @endif
                                    {{--<option @if(isset($companydomain->state) && $companydomain->state == 1) selected @endif value="1">{{__('Systems/SystemFive/companydomains.state_enabled')}}</option>--}}
{{--                                    <option @if(isset($companydomain->state) && $companydomain->state == 0) selected @endif value="0">{{__('Systems/SystemFive/companydomains.state_disabled')}}</option>--}}
                                </select>
                                {{--<input type="number" placeholder="entry_hour" class="form-control" @if(isset($companydomain->entry_hour)) value="{{ $companydomain->entry_hour }}"@endif name="entry_hour" placeholder="No entry_hour" required="">--}}
                            </div>
                        </div>
