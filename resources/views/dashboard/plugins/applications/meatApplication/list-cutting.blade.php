...
<div class="breadcrumbbar">
    <div class="row align-items-center">
        <div class="col-md-8 col-lg-8">
            <h4 class="page-title">{{ __('plugins/applications/meatApplication/meatCutting.cutList') }}</h4>
            <div class="breadcrumb-list">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{ __('side.cutting') }}</li>
                </ol>
            </div>
        </div>
        <div class="col-md-4 col-lg-4">
            <div class="widgetbar">
                <button  class="btn btn-primary-rgba" data-toggle="modal" data-target="#exampleStandardModal"><i class="feather icon-plus mr-2"></i>{{ __('plugins/applications/meatApplication/meatCutting.cutAdd') }}</button>
            </div>                        
        </div>
    </div>          
</div>
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-borderless" id="default-datatable">  
                            <thead>
                                <tr>
                                    <th>{{ __('plugins/applications/meatApplication/meatCutting.id') }}</th>
                                    
                                    <th>{{ __('plugins/applications/meatApplication/meatCutting.cutAR') }}</th>
                                    
                                    <th>{{ __('plugins/applications/meatApplication/meatCutting.cutEN') }}</th>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="button-list">
                                            ...
                                            <a href="{{ url('dashboard/plugins/systems/delete_cutting/'.$cut->id) }}" class="btn btn-danger-rgba" onclick="return confirm('Are you sure？')"><i class="feather icon-trash"></i></a>
                                        </div>
                                    </td>
                                </tr>
                                    ...
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="exampleStandardModalLabel">{{ __('plugins/applications/meatApplication/meatCutting.cutEdit') }}</h5>
                                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>
                                            <form action="{{ route('cuttings.update') }}" method="post">
                                            @csrf
                                            <div class="modal-body">
                                                <input type="hidden" name="cutting_method_id" value="{{ $cut->id }}">
                                                <label for="name_ar" class="col-form-label">{{ __('plugins/applications/meatApplication/meatCutting.cutAR') }}</label>
                                                <input type="text" name="name_ar" class="form-control" required="" value="{{ $cut->name_ar }}">
                                                <label for="name_en" class="col-form-label">{{ __('plugins/applications/meatApplication/meatCutting.cutEN') }}</label>
                                                <input type="text" name="name_en" class="form-control" required="" value="{{ $cut->name_en }}">
                                                <label for="price" class="col-form-label">{{ __('general.price') }}</label>
                                                <input type="number" step="any" id="price" name="price" class="form-control" required="" value="{{$cut->price}}" >
                                            </div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ __('plugins/applications/meatApplication/meatCutting.close') }}</button>
                                                <button type="submit" class="btn btn-primary">{{ __('plugins/applications/meatApplication/meatCutting.edit') }}</button>
                                            </div>
                                            </form>
                                        </div>
                                    </div>
...
<div class="modal-dialog" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="exampleStandardModalLabel">{{ __('plugins/applications/meatApplication/meatCutting.cutAdd') }}</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
            </button>
        </div>
        ...
        <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ __('plugins/applications/meatApplication/meatCutting.close') }}</button>
            <button type="submit" class="btn btn-primary">{{ __('plugins/applications/meatApplication/meatCutting.add') }}</button>
        </div>
        </form>
    </div>
</div>
